...
def fitness_func(solution, solution_idx):
    ...
    c_lib.c3540LoadInput(ffi.cast("bool *", in_arr.ctypes.data), len(in_arr))
    c_lib.c3540Propagate()
    node_activations = np.zeros(number_of_internal_nodes, dtype=np.bool8)
    c_lib.c3540GetNodeState(ffi.cast("bool *", node_activations.ctypes.data), len(node_activations))
...
# Use FFI
c_lib.c3540Init()
c3540_n_input = c_lib.c3540NumInput()
c3540_n_output = c_lib.c3540NumOutput()
c3540_n_wire = c_lib.c3540NumWire()

c3540_trans_prob = np.zeros(c3540_n_wire, dtype=np.float32)
c_lib.c3540GetTransitionProbability(ffi.cast("float *", c3540_trans_prob.ctypes.data), len(c3540_trans_prob))

c3540_P0 = np.zeros(c3540_n_wire, dtype=np.float32)
c_lib.c3540GetP0(ffi.cast("float *", c3540_P0.ctypes.data), len(c3540_P0))

c3540_P1 = np.zeros(c3540_n_wire, dtype=np.float32)
c_lib.c3540GetP1(ffi.cast("float *", c3540_P1.ctypes.data), len(c3540_P1))

number_of_inputs = c3540_n_input
number_of_outputs = c3540_n_output
number_of_internal_nodes = c3540_n_wire

transition_prob = c3540_trans_prob
trigger_false_prob = c3540_P0
trigger_true_prob = c3540_P1
