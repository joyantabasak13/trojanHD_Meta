...
def fitness_func(solution, solution_idx):
    ...
    c_lib.c5315LoadInput(ffi.cast("bool *", in_arr.ctypes.data), len(in_arr))
    c_lib.c5315Propagate()
    node_activations = np.zeros(number_of_internal_nodes, dtype=np.bool8)
    c_lib.c5315GetNodeState(ffi.cast("bool *", node_activations.ctypes.data), len(node_activations))
...
# Use FFI
c_lib.c5315Init()
c5315_n_input = c_lib.c5315NumInput()
c5315_n_output = c_lib.c5315NumOutput()
c5315_n_wire = c_lib.c5315NumWire()

c5315_trans_prob = np.zeros(c5315_n_wire, dtype=np.float32)
c_lib.c5315GetTransitionProbability(ffi.cast("float *", c5315_trans_prob.ctypes.data), len(c5315_trans_prob))

c5315_P0 = np.zeros(c5315_n_wire, dtype=np.float32)
c_lib.c5315GetP0(ffi.cast("float *", c5315_P0.ctypes.data), len(c5315_P0))

c5315_P1 = np.zeros(c5315_n_wire, dtype=np.float32)
c_lib.c5315GetP1(ffi.cast("float *", c5315_P1.ctypes.data), len(c5315_P1))

number_of_inputs = c5315_n_input
number_of_outputs = c5315_n_output
number_of_internal_nodes = c5315_n_wire

transition_prob = c5315_trans_prob
trigger_false_prob = c5315_P0
trigger_true_prob = c5315_P1
